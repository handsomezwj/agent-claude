...
- **8 个自定义工具（单 Agent 5 + 多 Agent 协作 3）**：
  ...
  - `troubleshoot` — 多 Agent 流水线排障：诊断 → 根因 → 方案三环接力，方案建议带破坏性词护栏双保险
  - `ops_report` — 多 Agent 主管-工人：主管拆活（状态/日志/风险）分给工人，收齐汇总成完整报告
  - `interview_prep` — 多 Agent 评审团：面试题发给原理/工程/面试官三专家，主席汇总满分答案
...
- **多 Agent 协作（三种模式）**：流水线接力（后环只拿前环输出）/ 主管-工人拆活 / 评审团多角度，内部每个 Agent 上下文隔离、任一环挂优雅降级不崩，封装成工具一句话触发（`CLAUDE_USE_MULTIAGENT` 可关）
...
- **自动化测试**：311 个 unittest 全绿，自研 FakeModel 假模型替身，零成本回归验证全部护栏
...
| `CLAUDE_USE_MULTIAGENT` | 是否启用多 Agent 协作工具（troubleshoot/ops_report/interview_prep） | `true` |
| `CLAUDE_USE_RELIABILITY` | 多 Agent 工具模型调用是否包上熔断 + 重试 | `true` |
| `CLAUDE_MODEL_RETRIES` | 单条模型调用重试次数（指数退避） | `2` |
| `CLAUDE_MODEL_BREAK_AFTER` | 连续失败多少次触发熔断跳闸 | `5` |
| `CLAUDE_MODEL_BREAK_RECOVER` | 熔断后冷却多少秒再试探 | `30` |
| `CLAUDE_USE_TRACE` | 多 Agent 协作跑完是否打印 trace 病历 | `true` |
| `CLAUDE_TRACE_FILE` | trace 病历存档文件（空 = 只打印不存档） | （空） |

## 项目结构

```
.
├── agent-claude.py    # 主程序：Agent 循环 + 工具系统 + 四层护栏 + 流式/MCP/RAG/长记忆 + 多 Agent 协作 + 企业级加固
...
│   ├── ops_pipeline.py    # 多 Agent 模式 · 流水线（诊断→根因→方案接力）
│   ├── ops_orchestrator.py# 多 Agent 模式 · 主管-工人（拆活 + 汇总）
│   ├── ops_debate.py      # 多 Agent 模式 · 评审团（多角度 + 主席汇总）
│   ├── multiagent_tools.py# 三模式封装成工具（troubleshoot/ops_report/interview_prep）
│   ├── reliability.py     # 可靠性层：重试 / 超时 / 熔断 / HardenedAgent
│   ├── observability.py   # 可观测性层：Tracer trace 病历 + TracedAgent
│   ├── prompting.py       # 提示词工程
...
│   ├── 25-enterprise.py   # 可靠性层示例（重试/超时/熔断五幕）
│   ├── 26-trace.py        # 可观测性示例（trace 病历五幕）
│   ├── 21-itops.py        # IT 运维专项示例（离线剧本演示护栏）
│   ├── *.py               # 每个能力的可运行示例
│   ├── test_*.py          # 311 个 unittest（FakeModel，零成本）
...
- 流式输出、MCP 协议、RAG 检索（词袋向量 + bge 语义向量）、真向量模型、多 Agent 协作（三种模式：流水线/主管-工人/评审团，接进成品）、提示词工程、长记忆持久化、异步编程、IT 运维排障（只读诊断 + 安全护栏）
- 企业级加固：可靠性层（重试 / 超时 / 熔断）透明包装进多 Agent 工具 + 可观测性层（trace 病历，谁/状态/耗时一眼定位）
...
311 个 unittest 全绿。测试不调用真实 API：用自研 FakeModel 假模型替身，几秒跑完、零成本。覆盖：命令黑名单（管道/分号拼接、大小写、`mkfs.*` 变体）、`../` 路径越权、日志越权拦截、服务三态、多 Agent 三种协作模式全流程与优雅降级、可靠性层（瞬时抖动被重试救回 / 一直坏被熔断拦住）、可观测性层（trace 病历的嵌套/状态/耗时/存档回放）。
